function open_comments(post_id, span){
    ...
    $.post("core/functions/ajax.php",{function: "get_comments", post_id: post_id},function(data){
		...
		$("#actual-comments").html('<div id = "actual-comments"><span class = "col-xs-12 no-padding"><span onclick = "close_comments(1, null)" class = "glyphicon pull-right glyphicon-remove comment-x"></span></span><textarea rows = "3" placeholder = "Write a comment..." class = "form-control" name ="comment" id = "commenttoget"></textarea><button class = "btn btn-info btn-block comment_submit" onclick = "submit_comment(' + post_id + ', this, 1)">COMMENT</button><span id = "comments">' + data + '</span></div>');
...
function show_comments(post_id, span){
    ...
    $.post("core/functions/ajax.php",{function: "get_comments", post_id: post_id},function(data){
		...
		$("#post"+post_id+"-bottom").html('<hr class = "replysharehr"><div class = "col-xs-12" id = "actual-comments"><span class = "no-padding"><span onclick = "close_comments(2, '+post_id+')" class = "glyphicon pull-right glyphicon-remove comment-x"></span></span><textarea rows = "2" placeholder = "Write a comment..." class = "form-control" name ="comment" id = "commenttoget'+post_id+'"></textarea><button class = "btn btn-info pull-right comment_submit" onclick = "submit_comment(' + post_id + ', this, 2)">COMMENT</button><br><br><span id = "comments'+post_id+'">' + data + '</span></div>');
...
function open_comments_share(post_id, span){
    $.post("core/functions/ajax.php",{function: "get_comments", post_id: post_id},function(data){
		...
		$("#post"+post_id+"-bottom-share").html('<hr class = "replysharehr"><div class = "col-xs-12" id = "actual-comments"><span class = "no-padding"><span onclick = "close_comments(2, '+post_id+')" class = "glyphicon pull-right glyphicon-remove comment-x"></span></span><textarea rows = "2" placeholder = "Write a comment..." class = "form-control" name ="comment" id = "commenttoget'+post_id+'"></textarea><button class = "btn btn-info pull-right comment_submit" onclick = "submit_comment(' + post_id + ', this, 2)">COMMENT</button><br><br><span id = "comments'+post_id+'">' + data + '</span></div>');
    ...
    $.post("core/functions/ajax.php",{function: "submit_comment", post_id: post_id, comment: comment},function(data){
		...
		$('#topalert').fadeIn("slow", function() { $(this).delay(2000).fadeOut("slow");
			...
			if(type == 1){
				...
				$('#comments').prepend('<span class = "row no-padding"><span class = "col-xs-12 a-comment"><span class = "row"><span class = "atext no-padding col-xs-12">' + comment + '<br><br></span><span class = "comment-username">Me</span></span></span></span>');
			...
			if(type == 2){
				...
				$('#comments' + post_id).prepend('<span class = "row no-padding"><span class = "col-xs-12 a-comment"><span class = "row"><span class = "atext no-padding col-xs-12">' + comment + '<br><br></span><span class = "comment-username">Me</span></span></span></span>');
